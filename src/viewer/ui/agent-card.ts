...
function moneyBlock(d: AgentDetail): string {
  const rows = [
    ...
    [
      'housing',
      `${d.housing.kind} · ${d.housing.due}c` +
  ...
  const rows = d.values.map((v) => {
    ...
    return `<tr>
      ...
      <td class="num ${drifted ? (v.drift > 0 ? 'up' : 'down') : 'dim'}">${
        drifted ? (v.drift > 0 ? '+' : '') + fmt(v.drift) : '·'
      }</td>
    </tr>`
  })
  return section(
    'values <span class="hint">effective · drift</span>',
    `<table class="values">${rows.join('')}</table>`,
  )
}

function vicesBlock(d: AgentDetail): string {
  const rows = d.vices.map(
    (v) =>
...
function needsBlock(d: AgentDetail): string {
  const rows = Object.entries(d.needs).map(
    ([k, v]) =>
...
function goalsBlock(d: AgentDetail): string {
  ...
  const goals = d.goals
    .map(
      (g) => `<li>${esc(g.kind)}${g.targetId ? ` → ${esc(g.targetId)}` : ''}
      <span class="dim">${Math.round(g.priority * 100)}%</span></li>`,
    )
    .join('')
  const cons = d.constraints.map((c) => `<li class="constraint">${esc(c)}</li>`).join('')
...
function relationshipsBlock(d: AgentDetail): string {
  ...
  const rows = d.relationships.map(
    (r) => `
    <tr>
...
function diariesBlock(d: AgentDetail): string {
  ...
  const entries = d.diaries.map(
    (e) => `<div class="diary"><div class="diary-day">day ${e.day}</div><p>${esc(e.text)}</p></div>`,
  )
